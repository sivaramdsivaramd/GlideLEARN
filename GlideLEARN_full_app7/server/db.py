...
# ---------------------------------------------------------
# Progress + streaks — derived entirely from study_plans, so a
# student's dashboard/progress/streak pages reflect real saved
# sessions rather than invented numbers.
# ---------------------------------------------------------
def distinct_subjects(user_id):
    """[(subject_id, subject_name), ...] this user has ever studied,
    most recently studied first."""
    rows = _conn.execute(
        """SELECT subject_id, subject_name, MAX(created_at) AS last_at
           FROM study_plans WHERE user_id = ?
           GROUP BY subject_id, subject_name
           ORDER BY last_at DESC""",
        (user_id,),
    ).fetchall()
    return [(r["subject_id"], r["subject_name"]) for r in rows]


def plans_for_subject(user_id, subject_id, limit=10):
    return _conn.execute(
        """SELECT * FROM study_plans WHERE user_id = ? AND subject_id = ?
           ORDER BY created_at ASC LIMIT ?""",
        (user_id, subject_id, limit),
    ).fetchall()


def all_plans(user_id, limit=50):
    return _conn.execute(
        "SELECT * FROM study_plans WHERE user_id = ? ORDER BY created_at ASC LIMIT ?",
        (user_id, limit),
    ).fetchall()
...
def _pct(row):
    ...
    return round(100 * (row["score_correct"] or 0) / total) if total else 0


def subject_progress(user_id, subject_id):
    """Returns {avgPct, attempts, trend:[pct,...]} for one subject."""
    rows = plans_for_subject(user_id, subject_id)
    if not rows:
        return {"avgPct": 0, "attempts": 0, "trend": []}
    trend = [_pct(r) for r in rows]
    return {"avgPct": round(sum(trend) / len(trend)), "attempts": len(rows), "trend": trend}


def overall_progress(user_id):
    rows = all_plans(user_id)
    if not rows:
        return {"avgPct": 0, "attempts": 0, "trend": []}
    trend = [_pct(r) for r in rows]
    return {"avgPct": round(sum(trend) / len(trend)), "attempts": len(rows), "trend": trend}
...
def _study_dates(rows):
    """Distinct calendar dates (as date objects) a set of study_plans
    rows were created on, sorted ascending."""
    dates = set()
    for r in rows:
        try:
            dates.add(_dt.datetime.fromisoformat(r["created_at"]).date())
        except ValueError:
            continue
    return sorted(dates)
...
def _current_streak(dates):
    """Consecutive-day streak ending at the most recent studied date
    (so it doesn't zero out just because 'today' hasn't happened yet
    in this demo's wiped-on-restart data)."""
    if not dates:
        return 0
    streak = 1
...
def subject_streak(user_id, subject_id):
    return _current_streak(_study_dates(plans_for_subject(user_id, subject_id, limit=200)))


def overall_streak(user_id):
    return _current_streak(_study_dates(all_plans(user_id, limit=500)))


# ---------------------------------------------------------
# Glide COINS — a lightweight reward currency derived entirely
# from saved study_plans (same "no invented numbers" approach as
# progress/streaks above), so it never needs its own ledger table:
#   - every saved session earns 10-30 coins, scaled by score
#   - the student's current overall streak adds a 5-coins-per-day bonus
# ---------------------------------------------------------
def _session_coins(row):
    return 10 + round(20 * _pct(row) / 100)


def user_coins(user_id):
    """Total Glide COINS for one user: {total, sessionCoins, streakBonus, streakDays}."""
    rows = all_plans(user_id, limit=1000)
    session_coins = sum(_session_coins(r) for r in rows)
    streak_days = overall_streak(user_id)
    streak_bonus = streak_days * 5
    return {
        "total": session_coins + streak_bonus,
...
def leaderboard(limit=100):
    """Every student account ranked by total Glide COINS, highest
    first. Returns [{userId, name, coins}, ...]."""
    students = _conn.execute(
        "SELECT id, full_name FROM users WHERE role = 'student'"
    ).fetchall()
    rows = [
        {"userId": s["id"], "name": s["full_name"], "coins": user_coins(s["id"])["total"]}
        for s in students
    ]
    rows.sort(key=lambda r: (-r["coins"], r["name"].lower()))
    return rows[:limit]
