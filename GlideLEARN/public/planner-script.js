...
(function () {
  ...
  // Click-ripple feedback on every button — delegated so it also
  // covers buttons rendered dynamically later (submit quiz, finish
  // unit, etc.). Needs the target to have position:relative and
  // overflow:hidden, which .btn-primary/.btn-secondary already do.
  ...
  // CSS animations only fire when the animation actually changes value,
  // so a static "shake" on .status-msg.error never replays when the
  // *text* changes but the class doesn't. Force a reflow between
  // removing and re-adding the class so every new error message
  // actually shakes into view instead of silently appearing once.
  ...
  function renderDashboard() {
    ...
    callApi("/api/dashboard-summary", null, "GET").then(function (data) {
      ...
      tileProgressMetric.innerHTML = '<span class="count-num"></span><span> % avg score</span>';
      animateCount(tileProgressMetric.querySelector(".count-num"), data.overallProgressPct, 800);
      tileStreakMetric.innerHTML = '<span class="count-num"></span><span> day streak</span>';
      animateCount(tileStreakMetric.querySelector(".count-num"), data.overallStreak, 800);
  ...
  document.querySelector('[data-next="duration"]').addEventListener("click", function () {
    const checked = document.querySelector('input[name="duration"]:checked');
    if (!checked) { setError(durationError, "Choose how long you want to study."); return; }

    let minutes, label;
    if (checked.value === "custom") {
      const rawValue = parseFloat(customDurationValue.value);
      if (!rawValue || rawValue <= 0) { setError(durationError, "Enter how long you want to study."); return; }
  ...
  document.querySelector('[data-next="subject"]').addEventListener("click", function () {
    ...
    if (!checked) { setError(subjectError, "Choose a subject to continue."); return; }
  ...
  document.querySelector('[data-next="unit"]').addEventListener("click", function () {
    ...
    if (!checked) { setError(unitError, "Choose a unit to continue."); return; }
  ...
  function startTopicQuiz() {
    ...
    callApi("/api/generate-topic-quiz", {
      ...
      if (result && result.success && result.quizzes && Object.keys(result.quizzes).length) {
        ...
        topicQuizContainer.innerHTML = "";
        setError(quizError, "AI question generation isn't available right now (check GROQ_API_KEY on the server). Try again in a moment.");
        submitQuizBtn.disabled = true;
      }
    });
  }
  ...
  function renderTopicQuiz() {
    ...
    Object.keys(state.quizByTopic).forEach(function (topic, topicIdx) {
      ...
      block.innerHTML =
        '<h3>' + escapeHtml(topic) + '</h3><p class="topic-progress">Topic ' + (topicIdx + 1) + ' of ' + Object.keys(state.quizByTopic).length + '</p>';

      state.quizByTopic[topic].forEach(function (q, qIdx) {
        const name = "t" + topicIdx + "_q" + qIdx;
        const qWrap = document.createElement("div");
        qWrap.className = "quiz-question";
        qWrap.style.animationDelay = (qIdx * 0.05) + "s";
        const p = document.createElement("p");
        p.className = "q-text";
        p.textContent = q.q;
        qWrap.appendChild(p);
        ...
        q.options.forEach(function (opt, optIdx) {
          ...
          optLabel.innerHTML =
            '<input type="radio" name="' + name + '" value="' + optIdx + '"> <span>' + escapeHtml(opt) + '</span>';
  ...
  submitQuizBtn.addEventListener("click", function () {
    ...
    if (unanswered > 0) {
      setError(quizError, "Please answer every question before continuing.");
      return;
    }
    quizError.textContent = "";
  ...
  function loadRankView() {
    ...
    callApi("/api/leaderboard", null, "GET").then(function (data) {
      ...
      data.leaderboard.forEach(function (row, i) {
        const li = document.createElement("li");
        li.className = "rank-row" + (row.userId === data.myUserId ? " is-you" : "") + (row.rank <= 3 ? " is-top" : "") + (row.rank === 1 ? " is-first" : "");
